...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (oneWayTab) {
        oneWayTab.addEventListener('click', () => {
            ...
            if (roundTripTab) roundTripTab.classList.remove('active-booking-tab');
    ...
    if (roundTripTab) {
        roundTripTab.addEventListener('click', () => {
            ...
            if (oneWayTab) oneWayTab.classList.remove('active-booking-tab');
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', (e) => {
            ...
            if (notificationModal) {
                ...
                // Fallback if modal somehow isn't found
                alert("Booking request submitted! We will contact you shortly. (This is a demo notification)");
            }
            
            bookingForm.reset();
            if (pickupCitySelect && destinationCitySelect && cities.length > 1) { // Reset dropdowns to default different values
                pickupCitySelect.value = cities[0].toLowerCase().replace(/\s+/g, '-');
                destinationCitySelect.value = cities[1].toLowerCase().replace(/\s+/g, '-');
            }
